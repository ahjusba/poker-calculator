...
describe('Ledger Utils - Integration Tests', () => {
  ...
  describe('calculatePayout', () => {
    it('should aggregate player balances correctly when player has multiple device IDs', async () => {
      const ledgerData = payout_data[0] as unknown as LedgerData;

      // Create players and link devices
      const akseli = await createPlayer('Akseli');
      await linkDeviceToPlayer('0InsxVr9xo', akseli.id);
      ...
      const janneP = await createPlayer('Janne P.');
      await linkDeviceToPlayer('zkGVJb7I5U', janneP.id);

      const jaakko = await createPlayer('Jaakko');
      await linkDeviceToPlayer('FQl3QSL8H6', jaakko.id);

      const kake = await createPlayer('Kake');
      await linkDeviceToPlayer('lqP155LHFZ', kake.id);

      const lauriP = await createPlayer('Lauri P.');
      await linkDeviceToPlayer('7C7WggNp1M', lauriP.id);

      // Call aggregatePlayerBalances
      const playerBalances = await aggregatePlayerBalances(ledgerData);

      // Sort by name for consistent comparison
      const sortedBalances = playerBalances.sort((a, b) => a.name.localeCompare(b.name));

      // Expected balances
      const expectedBalances = [
        { name: 'Akseli', net: -4000 },    // -2000 + -2000 (two devices)
        { name: 'Jaakko', net: 4132 },     // +4132 (one device)
        { name: 'Janne P.', net: -176 },   // -176 (one device)
        { name: 'Jussi', net: -2000 },     // -2000 (one device)
        { name: 'Kake', net: 2011 },       // +2011 (one device)
        { name: 'Lauri P.', net: 33 }      // +33 (one device)
      ].sort((a, b) => a.name.localeCompare(b.name));

      // Verify the aggregation
      expect(sortedBalances).toHaveLength(expectedBalances.length);

      sortedBalances.forEach((balance, index) => {
        expect(balance.name).toBe(expectedBalances[index].name);
        expect(balance.net).toBe(expectedBalances[index].net);
      });

      // Specifically verify Akseli has aggregated amount from both devices
      const akseliBalance = sortedBalances.find(b => b.name === 'Akseli');
      expect(akseliBalance).toBeDefined();
      expect(akseliBalance!.net).toBe(-4000); // -2000 + -2000 from two devices
    });
    ...
    it('should calculate correct payout from payout_data fixture', async () => {
      const ledgerData = payout_data[0] as unknown as LedgerData;
